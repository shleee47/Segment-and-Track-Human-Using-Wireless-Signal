...
#        x = self.logmel_extractor(x)    # (batch_size, 1, time_steps, mel_bins)
        ...
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg') ## T/2
        x = F.dropout(x, p=0.2, training=self.training, inplace=True)
        x = self.resnet(x) ## T / 16
        x = F.avg_pool2d(x, kernel_size=(2, 2)) ## T/32
        x = F.dropout(x, p=0.2, training=self.training, inplace=True)
        x = self.conv_block_after1(x, pool_size=(1, 1), pool_type='avg') ## T/32
        x = F.dropout(x, p=0.2, training=self.training, inplace=True)
        x = torch.mean(x, dim=3)
